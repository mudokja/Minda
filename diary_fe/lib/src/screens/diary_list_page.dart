  ...
  try {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Failed to fetch diary entries. Please try again later.'),
        actions: <Widget>[
          TextButton(
            child: const Text('OK'),
              ...
              children: <Widget>[
                ...
                Padding(
                  ...
                  child: Row(
                    children: <Widget>[
                      IconButton(
                        onPressed: () => CalendarDialog.showCalendarDialog(context),
                        icon: Image.asset('assets/icon/white_calendar.png', width: 45, height: 45),
                      ),
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                            ...
                            filled: true,
                            contentPadding: const EdgeInsets.symmetric(vertical: 10.0).copyWith(left: 20.0),
                            ...
                            suffixIcon: const Icon(Icons.search_rounded, color: Colors.white, size: 30),
                          ),
                        ),
                      ),
                    ],
                  ...
                  : Container(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          DiaryEntry entry = currentPageEntries[index]; // 현재 페이지의 일기 항목
                          double ratio = index / (currentPageEntries.length - 1);
                          Color color = ratio <= 0.5 ? const Color(0xfff5b9d0) : const Color(0xff7769D4);
                          ...
                          return GestureDetector(
                            onTap: () {
                              ...
                              //       selectedDay: DateTime.parse(currentPageEntries[index].split('\n')[0]),
                              //     ),
                              // Navigate to DiaryPage with selected date
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => DiaryListPage(
                                    // selectedDay: DateTime.parse(currentPageEntries[index].split('\n')[0]),
                                    // selectedDay: DateTime.parse(currentPageEntries[index].diarySetDate),  // 'diarySetDate'가 문자열이어야 합니다.
                                  selectedDay: DateTime.parse(entry.diarySetDate), // 일기 날짜를 DateTime 객체로 변환
                                  ),
                                ),
                              );
                            },
                            child: Column(
                              ...
                              children: [
                                ...
                                Padding(
                                  ...
                                  child: Text(
                                    // currentPageEntries[index].split('\n')[1],
                                    entry.diaryTitle ?? 'No Title', // 제목 직접 사용
                    ...
                    child: GestureDetector(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.bold,
                          color: _currentPageIndex == index ? const Color(0xff7769D4) : Colors.white,
                        ),
                        ),
                      ),
                    ),
